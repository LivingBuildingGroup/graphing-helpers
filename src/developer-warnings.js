...
const consoleDeveloperWarnings =(props, options={}) => {
  const expected = {
    ...
    legendUnits: {
      type: 'object',
      required: true,
      notes: 'required for axes and grouping of selectors',
    },
    legendAbbrevs: {
      type: 'object',
      required: 'preferred',
      notes: 'labels or units will be used if legendAbbrevs is not provided',
    },
    legendLabels: {
      type: 'object',
      required: 'preferred',
    ...
    legendDefinitions: {
      type: 'object',
      required: 'preferred',
      notes: 'future incorporation',
    },
    legendDescription: {
      type: 'object',
      required: false,
      notes: 'currently only used to describe platform events',
    },
    dataType: {
      type: 'number',
      required: false,
  ...
  for(let prop in expected){
    ...
    const method = 
      theProp.required === true && problem ?
        'error' :
        theProp.required && problem ?
          'warn' :
          problem && options.all ?
            'log' :
            null;
    if(method){
      console[method](prop, theProp.type, theProp.notes, 'you sent: ', Array.isArray(youSent) ? 'array' : typeof youSent, youSent);
    }
  }
};
